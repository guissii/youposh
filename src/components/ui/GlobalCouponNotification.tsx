...
export function GlobalCouponNotification() {
  ...
  useEffect(() => {
    ...
    // Check if dismissed for THIS specific coupon
    const activeCode = settings.activeGlobalCoupon || "default";
    if (localStorage.getItem(`couponDismissed_${activeCode}`) === "true") {
      return;
    }
  ...
  const handleDismiss = () => {
    setIsVisible(false);
    const activeCode = settings.activeGlobalCoupon || "default";
    localStorage.setItem(`couponDismissed_${activeCode}`, "true");
  };
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div 
          ...
          className="fixed bottom-24 sm:bottom-6 left-4 right-4 sm:right-auto sm:left-6 z-[9999] sm:max-w-[320px] w-auto mx-auto sm:mx-0"
        >
