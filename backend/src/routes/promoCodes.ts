...
router.post('/validate', async (req, res) => {
    try {
        ...
        if (promo.startDate && new Date(promo.startDate) > new Date()) return res.status(400).json({ error: 'Code promo pas encore actif' });
        ...
        if (typeof orderTotal === 'number' && promo.minOrder > 0 && orderTotal < promo.minOrder) return res.status(400).json({ error: `Commande minimum: ${promo.minOrder} MAD` });
